...
class TestUserInfoRestApi(unittest.TestCase):
    ...
    def test_02_get_all_users(self):
        ...
        self.user_data['account_id'] = user['account_id']

    def test_03_get_user(self):
        res = self.client.get(f'/users/{self.user_data["account_id"]}')
        user = json.loads(res.data)
        self.assertEqual(self.user_data['account_id'], user['account_id'])
        self.assertEqual(self.user_data['first_name'], user['first_name'])
        self.assertEqual(self.user_data['last_name'], user['last_name'])
        self.assertEqual(self.user_data['email'], user['email'])
        self.assertEqual(self.user_data['password_hash'], user['password_hash'])
    ...
    def test_04_update_user(self):
        ...
        self.user_data['email'] = 'john_lee@zmail.com'
        ...
        res = self.client.put(f'/users/{self.user_data["account_id"]}', data=self.user_data)
        user = json.loads(res.data)
        self.assertEqual(self.user_data['account_id'], user['account_id'])
        self.assertEqual(self.user_data['first_name'], user['first_name'])
        self.assertEqual(self.user_data['last_name'], user['last_name'])
    ...
    def test_05_remove_user(self):
        res = self.client.delete(f'/users/{self.user_data["account_id"]}')
