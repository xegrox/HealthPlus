...
user_nric_map = BasicDatabase('user_nric_map')
...
def __check_user_exists(account_id):
    with user_accounts.open() as accounts:
...
def __update(account_id, data):
    with user_accounts.open() as accounts:
        ...
        if val := data.get('nric'):
            with user_nric_map.open() as nric_map:
                if val in nric_map:
...
def create(nric, first_name, last_name, password):
    with user_accounts.open() as accounts:
        with user_nric_map.open() as nric_map:
            if nric in nric_map:
                raise UserAlreadyExistsError()  # Abort if nric already exists
